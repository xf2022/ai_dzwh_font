<script lang='ts'>
    ...
    let yesterday_sessions: Session[] = [];
    let week_sessions: Session[] = [];
    let month_sessions: Session[] = [];
    let other_sessions: Session[] = [];
       ...
       <ul class="p-4">
           <li>
               <button class="flex items-center gap-x-3 py-2 px-3 text-sm text-slate-700 rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300 border-t border-b border-gray-200 dark:border-gray-700 w-full"
                   ...
                   <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" width="16" height="16" stroke-width="1.5"
           ...
           {#if today_sessions.length > 0}
               ...
               {#each today_sessions as session}
                   <li class="flex items-center gap-x-3 py-2 px-3 text-lg rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300"
                       class:chosen-session={currSid === session?.sid} >
                       <div class="relative flex w-full" on:click={() => clickSession(session)}>
                           {session?.name}
                       </div>
                   </li>
               {/each}
           ...
           { #if yesterday_sessions.length > 0}
               <li class="text-xs text-left px-3 mt-5 font-bold">
                   <span>昨天</span>
               </li>
               {#each yesterday_sessions as session}
                   <li class="flex items-center gap-x-3 py-2 px-3 text-lg rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300"
                       class:chosen-session={currSid === session?.sid} >
                       <div class="relative flex w-full" on:click={() => clickSession(session)}>
                           {session?.name}
           ...
           { #if month_sessions.length > 0 }
               <li class="text-xs text-left px-3 mt-5 font-bold">
               <span>前30天</span>
               </li>
               {#each month_sessions as session}
                   <li class="flex items-center gap-x-3 py-2 px-3 text-lg rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300"
                       class:chosen-session={currSid === session?.sid} >
                       <div class="relative flex w-full" on:click={() => clickSession(session)}>
                           {session?.name}
           ...
           { #if other_sessions.length > 0 }
               ...
               {#each other_sessions as session}
                   <li class="flex items-center gap-x-3 py-2 px-3 text-lg rounded-md hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300"
                       class:chosen-session={currSid === session?.sid} >
                       <div class="relative flex w-full" on:click={() => clickSession(session)}>
                           {session?.name}
           ...
           <li class="relative"> <!-- 相对定位，确保模态框基于按钮定位 -->
               ...
               <div class="flex justify-center gap-x-1 w-full">
                   <button class="px-4 py-2 bg-blue-500 text-white rounded-md text-sm hover:bg-blue-600 transition duration-200" on:click={generateReport}>
                       确定
                   </button>

                   <!-- 取消按钮 -->
                   <button class="px-4 py-2 border border-gray-300 text-gray-700 rounded-md text-sm hover:bg-gray-100 transition duration-200" on:click={cancelReport}>
                       取消
                   </button>
               </div>
               {/if}
           ...
           <li class="relative"> <!-- 相对定位，确保模态框基于按钮定位 -->
               ...
               {#if showConfirm_ppt}
                   ...
                   <!-- 按钮容器，居中并增加按钮间距 -->
                   <div class="flex justify-center gap-x-1 w-full">
                       <button class="px-4 py-2 bg-blue-500 text-white rounded-md text-sm hover:bg-blue-600 transition duration-200" on:click={generatePPT}>
                           确定
                       </button>

                       <!-- 取消按钮 -->
                       <button class="px-4 py-2 border border-gray-300 text-gray-700 rounded-md text-sm hover:bg-gray-100 transition duration-200" on:click={cancelReport_ppt}>
                           取消
                       </button>
                   </div>
               {/if}
